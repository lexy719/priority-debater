...
/**
 * PD Commerce scan engine — OpenAI only.
 *
 * `runScan(url)` produces a full `CommerceReport`:
 *   fetchStore → inferContext → buildQueries → runBuyerTest (OpenAI web search)
 *   → scoreStore → deriveFixes.
 *
 ...
 * renders, honestly labelled. The brand-parsing / store-mention / competitor-
 * ranking logic is carried forward from the previous multi-provider engine,
 * narrowed to a single provider per the all-OpenAI decision.
 */
...
export async function runScan(input: string): Promise<CommerceReport> {
  ...
  const [bt, googleCheck] = await Promise.all([
    runBuyerTest(profile, specs, ctx.category),
    runGoogleCheck(profile, ctx),
  ]);
  ...
  return {
    ...
    fixes: deriveFixes(profile, bt, scores),
    unlocked: false,
