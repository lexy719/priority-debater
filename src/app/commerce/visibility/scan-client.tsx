...
/**
 * The scan itself. Split out of page.tsx so the route can be a server
 * component that reads ?url= — a visitor who types their domain into the
 * Commerce hero lands here with the audit already running, instead of being
...
export default function VisibilityScan({ initialUrl = "", initialReport = null, initialError = null }: {
  initialUrl?: string; initialReport?: Report | null; initialError?: string | null;
}) {
  const [url, setUrl] = useState(initialUrl);
  const [busy, setBusy] = useState(false);
  const [err, setErr] = useState<string | null>(null);
  // A report handed down from the server is already the answer: arriving with
  // ?url= renders the finished audit in the HTML itself, so it is readable
  // with JavaScript off and by the very agents this product is about.
  ...
  const run = async (value: string) => {
    ...
    try {
      const r = await fetch("/api/commerce/visibility", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ url: v }) });
  ...
  const onSubmit = (e: React.FormEvent) => { e.preventDefault(); void run(url); };
  // The server's failure stands until the visitor tries something else.
  const shownErr = err ?? (report ? null : initialError);
    ...
    <main style={{ backgroundColor: PAPER, color: INKB, minHeight: "100dvh", fontFamily: "var(--app-font-sans), system-ui, sans-serif" }}>
      <div className="mx-auto max-w-[1040px] px-6 pb-24">
        ...
        <section className="pt-12">
          ...
          <form onSubmit={onSubmit} className="mt-8 flex flex-wrap items-center gap-3">
            <input
          ...
          {shownErr && <div className="mt-3 text-[13px] font-semibold" style={{ color: FAULTB }}>{shownErr}</div>}
          {busy && (
