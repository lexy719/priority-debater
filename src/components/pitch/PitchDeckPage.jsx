...
function Slide({ s, theme, project, footerByline }) {
...
/**
 * Emergent `PitchDeck.jsx` layout — props supply `deck` / `project` from session or static `studioData`.
 */
          ...
          <div className="grid gap-8 lg:grid-cols-12">
            ...
            <div className="lg:col-span-4 lg:self-end">
              <p className="max-w-md font-mono text-sm leading-relaxed text-white/65">
                Auto-generated investor-grade deck. Headline metric on every slide. Branded with your kit. Export to PDF
                or Keynote.
              </p>
        ...
        <div className="mx-auto max-w-[1480px] px-6 lg:px-10">
          <div className="flex flex-wrap items-center justify-between gap-3 border-2 border-b-0 border-black bg-black px-4 py-2 text-white">
            ...
            <div className="flex items-center gap-2">
              ...
              <button type="button" className="border border-white/30 px-2 py-1.5 text-white/60 hover:text-white">
          ...
          <div className="relative border-2 border-black bg-black/5">
            <div className="aspect-[16/9] w-full">
              <Slide s={slide} theme={theme} project={project} footerByline={footerByline} />
            </div>
            ...
            {deck.map((s, i) => (
              <button
                ...
                className={`group border-2 p-3 text-left transition hover:-translate-y-0.5 ${
                  i === idx ? "border-black bg-[var(--hi)] shadow-brutal-sm" : "border-black/30 bg-white hover:border-black"
                }`}
              >
                <div className="font-display text-xl">{s.no}</div>
                <div className="mt-1 font-mono text-[9px] tracking-wider text-neutral-700">{s.kicker}</div>
                <div className="mt-2 line-clamp-2 font-display text-[11px] leading-tight">{s.title}</div>
              </button>
            ))}
          </div>
        </div>
          ...
          <div className="mt-8 grid gap-6 lg:grid-cols-12">
            ...
            <div className="lg:col-span-5">
              <div className="border-2 border-black bg-black p-6 text-white">
                ...
                <p className="mt-3 font-display text-lg leading-snug">&ldquo;{talkTrackObjection}&rdquo;</p>
                <div className="mt-5 font-mono text-[10px] tracking-wider text-white/55">YOUR PRE-DEBATED ANSWER</div>
                <p className="mt-2 font-mono text-xs leading-relaxed text-white/80">&ldquo;{talkTrackAnswer}&rdquo;</p>
              </div>
            </div>
          </div>
        </div>
