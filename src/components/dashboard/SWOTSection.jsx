...
const Block = ({ title, items, tone, Icon, testid }) => {
    const dark = tone === "ink";
    const visibleItems = items.length > 0 ? items : [`No explicit ${title.toLowerCase()} were named in this report.`];
    return (
        <div
            data-testid={testid}
            className={`relative border-2 border-black p-6 hover-lift ${dark ? "bg-black text-white" : "bg-white"}`}
        >
            ...
            <ul className="mt-5 space-y-3">
                {visibleItems.map((t, i) => (
                    <li key={i} className="flex gap-3 font-mono text-[12.5px] leading-relaxed">
                        <span className={dark ? "text-[var(--hi)]" : "text-black"}>{String(i + 1).padStart(2, "0")}</span>
...
export default function SWOTSection() {
    const { swot, dashboardUi } = useResultsDashboard();
            ...
            <div className="relative mx-auto max-w-[1480px] px-6 lg:px-10">
                <div className="mb-12 grid gap-8 lg:grid-cols-12">
                    <div className="lg:col-span-7">
                        <div className="font-mono text-[10px] tracking-wider text-neutral-500">{dashboardUi.swot.eyebrow}</div>
                    ...
                    <div className="lg:col-span-5 lg:pt-6">
                        <p className="font-mono text-sm leading-relaxed text-neutral-600">
                            Strengths, weaknesses, opportunities, and threats are written as a decision map for this idea:
                            what is working, what is fragile, where momentum exists, and what could break the case.
                        </p>
                    </div>
                </div>
