...
import {
  ...
  ArrowRight, Skull, ChevronRight, Activity, DollarSign, Flame,
} from "lucide-react";
import { DEFAULT_REPORT, type Report, type Tone, type Severity, type Priority } from "@/lib/report";

export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Priority Debater — Validation Report" },
      { name: "description", content: "Investor-grade idea validation." },
    ],
  }),
  component: () => <ResultsPage report={DEFAULT_REPORT} />,
});
...
export function ResultsPage({ report = DEFAULT_REPORT }: { report?: Report }) {
  const [tab, setTab] = useState<TabId>("overview");

  return (
    <div className="min-h-screen bg-background text-foreground">
      <Ticker items={report.ticker} />
      <TopBar meta={report.meta} />
      <Hero report={report} />
      <ExecSummaryBand data={report.execSummary} />
      <TabBar tab={tab} setTab={setTab} />
      <main className="mx-auto max-w-[1480px] px-4 md:px-8 pb-32">
        {tab === "overview"    && <Overview data={report.overview} score={report.score.value} />}
    ...
    <section className="bg-ink text-ink-foreground border-y border-ink grid-bg-ink">
      <div className="mx-auto max-w-[1480px] px-4 md:px-8 py-20 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
          <button className="px-6 py-4 bg-accent text-accent-foreground font-display text-sm tracking-widest hover:opacity-90 flex items-center gap-2">
            {data.primaryCta} <ArrowRight className="size-4" />
          </button>
          <button className="px-6 py-4 border border-ink-foreground/40 text-ink-foreground font-display text-sm tracking-widest hover:bg-ink-foreground/5">
            {data.secondaryCta}
          </button>
        </div>
      </div>
...
function Ticker({ items }: { items: Report["ticker"] }) {
  ...
  return (
    <div className="bg-ink text-ink-foreground border-y border-ink overflow-hidden">
...
function TopBar({ meta }: { meta: Report["meta"] }) {
  return (
    <div className="border-b border-border bg-background/80 backdrop-blur sticky top-0 z-40">
      <div className="mx-auto max-w-[1480px] px-4 md:px-8 py-4 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-2 text-[10px] tracking-widest">
          ...
          <span>·</span><span className="text-muted-foreground">DEBATE</span>
        ...
        <div className="flex items-center gap-2">
          <a href="/debate" className="px-3 py-2 border border-border text-[10px] tracking-widest hover:bg-surface">DEBATE MODE</a>
          <button className="px-3 py-2 bg-accent text-accent-foreground text-[10px] tracking-widest font-bold hover:opacity-90">+ RE-RUN VALIDATION</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="border border-ink-foreground/25 bg-ink-foreground/5 p-6 self-start">
      ...
      <div className="flex items-end gap-4">
        <div className="font-display text-[8rem] leading-none text-data">{score.value}</div>
...
function Overview({ data, score }: { data: Report["overview"]; score: number }) {
  return (
    <>
  ...
  return (
    <>
      ...
      <div className="mt-6 ip-card divide-y divide-border">
        {data.items.map((a, i) => (
          <div key={a.title} className="grid grid-cols-[60px_120px_140px_1fr_100px] gap-4 px-5 py-4 items-center hover:bg-surface-2/40">
            <div className="font-display text-2xl text-muted-foreground">{String(i + 1).padStart(2, "0")}</div>
            <div>
              <span className={`text-[10px] px-2 py-1 border tracking-widest font-bold ${prioBorder(a.p)}`}>● {a.p}</span>
            </div>
            <div className="text-[10px] tracking-widest text-muted-foreground">{a.w}</div>
            <div>
