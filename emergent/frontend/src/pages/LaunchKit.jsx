...
import { ArrowRight, Check, Megaphone, Mail, MessageSquare, Hash, Tag, Zap } from "lucide-react";
        ...
        <div className="max-w-[1400px] mx-auto px-5 lg:px-8 grid lg:grid-cols-12 gap-10 items-end">
          <div className="lg:col-span-8">
            ...
            <p className="mt-5 max-w-xl text-white/60 font-body leading-relaxed">
              Insight doesn&apos;t pay. Customers do. Everything below is built to
              be copy-pasted and shipped — your goal is your first paying user in{" "}
          ...
          <div className="lg:col-span-4 grid grid-cols-2 gap-3">
            {[
              ...
              { k: "Channels", v: "3", c: "#007aff" },
              { k: "Outreach assets", v: "30", c: "#ffd60a" },
      ...
      <main className="max-w-[1400px] mx-auto px-5 lg:px-8 py-16 lg:py-20 space-y-10">
        ...
        <motion.section initial="hidden" whileInView="show" viewport={viewport} variants={reveal}>
          ...
          <div className="mt-4 grid lg:grid-cols-12 gap-6">
              ...
              <div className="space-y-5">
                <div className="border border-black/15 p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <CopyButton text={PRODUCT_PAGE.headline} testid="copy-headline" />
                  </div>
                  <p className="font-display text-3xl uppercase tracking-tight leading-none">{PRODUCT_PAGE.headline}</p>
                  <p className="mt-3 text-sm text-black/70 font-body leading-relaxed">{PRODUCT_PAGE.subhead}</p>
                </div>
                <div className="grid sm:grid-cols-3 gap-3">
                  {PRODUCT_PAGE.valueProps.map((vp, i) => (
            ...
            {/* live preview */}
            <Card className="lg:col-span-5 bg-[#6B1F2A] border-black text-[#F3EEE3]">
              <span className="font-mono text-[9px] uppercase tracking-[0.18em] text-[#F3EEE3]/50">
                Product page · preview
              </span>
              <div className="mt-4">
                <span className="font-mono text-[9px] uppercase tracking-[0.3em] text-[#C8A24B]">MINUTA</span>
                <h3 className="mt-3 font-display text-3xl uppercase leading-none">{PRODUCT_PAGE.headline}</h3>
                <p className="mt-3 text-xs text-[#F3EEE3]/70 font-body leading-relaxed">{PRODUCT_PAGE.subhead}</p>
                <div className="mt-5 flex flex-wrap gap-2">
                  <span className="bg-[#C8A24B] text-[#14110F] font-mono text-[10px] uppercase tracking-[0.15em] px-4 py-2.5">
                    {PRODUCT_PAGE.primaryCta}
                  </span>
                  <span className="border border-[#F3EEE3]/40 font-mono text-[10px] uppercase tracking-[0.15em] px-4 py-2.5">
                    {PRODUCT_PAGE.secondaryCta}
                  </span>
                </div>
                <div className="mt-6 grid grid-cols-3 gap-2">
                  {PRODUCT_PAGE.pricing.map((p) => (
                    <div
                      key={p.name}
                      className={`p-2.5 border ${p.featured ? "border-[#C8A24B] bg-[#C8A24B]/10" : "border-[#F3EEE3]/20"}`}
                    >
                      <p className="font-mono text-[8px] uppercase tracking-[0.15em] text-[#F3EEE3]/50">{p.name}</p>
                      <p className="font-display text-xl mt-1">{p.price}</p>
                      <p className="font-mono text-[8px] text-[#F3EEE3]/45">{p.per || p.note}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {ACQUISITION.map((c) => (
              <Card key={c.n} className="flex flex-col">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-mono text-[9px] uppercase tracking-[0.18em] bg-black text-white px-2 py-1">
                    Channel
                  </span>
                </div>
        ...
        <motion.section initial="hidden" whileInView="show" viewport={viewport} variants={reveal}>
          ...
          <div className="mt-4 grid lg:grid-cols-12 gap-6">
            <Card className="lg:col-span-5 bg-black text-white border-black">
              ...
              <p className="mt-3 text-sm text-white/70 font-body leading-relaxed">{OFFER.founding}</p>
              <div className="mt-5 border-t border-white/15 pt-4 space-y-3">
                <div>
                  ...
                  <p className="text-xs text-white/70 font-body mt-1">{OFFER.urgency}</p>
                </div>
                <div>
                  <p className="font-mono text-[9px] uppercase tracking-[0.18em] text-[#32d74b]">Risk reversal</p>
                  <p className="text-xs text-white/70 font-body mt-1">{OFFER.guarantee}</p>
                </div>
              </div>
            </Card>
            <Card className="lg:col-span-7">
              <div className="flex items-center justify-between mb-3">
                ...
                <CopyButton text={OFFER.beta} testid="copy-beta" />
              </div>
              <div className="border border-black/15 bg-[#f4f4f0] p-4">
                <p className="text-sm text-black/75 font-body leading-relaxed whitespace-pre-line">{OFFER.beta}</p>
              </div>
