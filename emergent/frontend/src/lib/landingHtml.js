import { BRAND, PRODUCT_PAGE } from "@/lib/flowData";
...
export const ARCHETYPES = [
  ...
  { id: "editorial", name: "Editorial Serif", desc: "Premium, on the MINUTA brand palette." },
];
...
export function buildLandingHTML(archetypeId = "brutalist") {
  ...
  const tiers = p.pricing
    .map(
      ...
      <div class="card tier ${pr.featured ? "tier--hot" : ""}">
        ...
        <span class="tname">${pr.name}</span>
        <div class="price">${pr.price}<small>${pr.per || ""}</small></div>
        <span class="tnote">${pr.note}</span>
        <a class="btn btn--block" href="#cta">${pr.cta}</a>
      </div>`
    )
    .join("");
  ...
  <div class="wrap">
    <header>
      ...
      <nav>
        ...
        <a class="btn" href="#cta" style="padding:10px 18px;">${p.secondaryCta}</a>
      </nav>
    </header>

    <section class="hero">
      <h1>${p.headline.replace(/\.$/, "")}<span class="accent">.</span></h1>
      <p>${p.subhead}</p>
      <div class="actions">
        <a class="btn" href="#cta">${p.primaryCta}</a>
        <a class="btn btn--ghost" href="#features">${p.secondaryCta}</a>
      </div>
    </section>
    ...
    <section id="cta" class="cta">
      <h2>Stop drafting from scratch.</h2>
      <a class="btn" href="mailto:hello@${name.toLowerCase()}.legal">${p.primaryCta}</a>
    </section>

    <footer>
      <span>© ${new Date().getFullYear()} ${name}. Legal drafting, native.</span>
