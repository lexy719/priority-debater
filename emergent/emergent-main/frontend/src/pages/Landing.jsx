...
import {
    Eye, EyeOff, Monitor, Smartphone, Download, Sparkles,
    ArrowUp, ArrowDown, Pencil, Lock, Palette, X,
} from "lucide-react";
import { toast, Toaster } from "sonner";

const PRESET_ACCENTS = ["#7dd3fc", "#38bdf8", "#ffe600", "#ff3b30", "#14b870", "#ff2d87"];

// Themed preview component — renders the generated landing page
function PreviewFrame({ theme, hero, sections, device, accent }) {
        ...
        >
            ...
            <div className="flex items-center justify-between border-b border-current/15 px-5 py-3">
                ...
                <div className="hidden gap-5 font-mono text-[9px] tracking-wider opacity-70 sm:flex">
                    <span>PRODUCT</span><span>PILOTS</span><span>PRICING</span><span>CONTACT</span>
                </div>
                <button className="border px-3 py-1 font-mono text-[9px] tracking-wider" style={{ borderColor: theme.fg }}>
                    {hero.cta}
                </button>
            </div>
            ...
            {visible.map((s) => {
                if (s.id === "hero") return (
                    <div key="hero" className="border-b border-current/10 px-5 py-12 sm:px-10 sm:py-20 lg:py-24">
                        <div className="inline-block border px-2 py-0.5 font-mono text-[10px] tracking-wider opacity-80" style={{ borderColor: theme.fg }}>
                            {hero.kicker}
                        </div>
                        <h1 className={`mt-6 font-display leading-[0.85] tracking-tight ${
                            device === "mobile" ? "text-5xl" : "text-6xl sm:text-7xl lg:text-[112px]"
                        }`}>
                            {hero.title.split(". ").map((seg, i, arr) => (
                                <span key={i} className="block">
                                    {i === arr.length - 2 ? (
                                        <span style={{ background: accent, color: accentText, padding: "0 0.08em", boxDecorationBreak: "clone" }}>
                                            {seg}.
                                        </span>
                                    ) : (
                                        seg + (i < arr.length - 1 ? "." : "")
                        ...
                        <p className="mt-8 max-w-xl font-mono text-sm leading-relaxed opacity-70 sm:text-base">
                            {hero.sub}
                        </p>
                        <div className="mt-9 flex flex-wrap items-center gap-3">
                            <button className="border-2 border-current px-6 py-3 font-mono text-xs tracking-wider" style={{ background: theme.fg, color: theme.bg }}>
                                {hero.cta} →
                            </button>
                            <button className="border-2 border-current px-6 py-3 font-mono text-xs tracking-wider">
                                SEE THE NUMBERS
                            </button>
                ...
                if (s.id === "logos") return (
                    <div key="logos" className="border-y border-current/10 px-5 py-6 text-center font-mono text-[10px] tracking-wider opacity-60">
                        <span className="mr-3 opacity-50">TRUSTED BY OPERATORS AT</span>
                        DHL · POSTNL · MIROR · COLIS PRIVÉ · FEDEX EU · +3 IN PILOT
                    </div>
                );

                if (s.id === "problem") return (
                    <div key="problem" className="border-t border-current/10 px-5 py-12 sm:px-10 sm:py-16">
                        ...
                        <div className="mt-3 grid gap-6 lg:grid-cols-3">
                            ...
                            <p className="lg:col-span-2 font-mono text-sm leading-relaxed opacity-75">
                                EU ICE bans hit 18 city centres by 2027. Same-day expectations are now baseline.
                                Diesel TCO is up 34% YoY. The economics of last-mile have already inverted —
                                and incumbents are stuck on three-year refresh cycles.
                            </p>
                        </div>
                    </div>
                );

                if (s.id === "product") return (
                    <div key="product" className="border-t border-current/10 px-5 py-12 sm:px-10 sm:py-16">
                        ...
                        <h2 className="mt-3 font-display text-3xl leading-[0.95] sm:text-4xl lg:text-5xl">
                            ONE STACK. <span style={{ background: accent, color: accentText, padding: "0 0.08em" }}>SIX MOVING PARTS.</span>
                        </h2>
                            ...
                            {landingFeatures.map((f) => (
                                <div key={f.title} className="p-5" style={{ background: theme.bg }}>
                                    <div className="flex items-center justify-between">
                                        <span className="border px-2 py-0.5 font-mono text-[9px] tracking-wider" style={{ borderColor: theme.fg }}>{f.kicker}</span>
                ...
                if (s.id === "metrics") return (
                    <div key="metrics" className="border-t border-current/10 px-5 py-12 sm:px-10 sm:py-16" style={{ background: "#0a0a0a", color: "#fff" }}>
                        ...
                        <div className="mt-7 grid grid-cols-2 gap-px border border-white/20 bg-white/15 lg:grid-cols-4">
                            {landingMetrics.map((m) => (
                                <div key={m.l} className="bg-black p-5">
                                    <div className="font-display text-5xl" style={{ color: accent }}>{m.v}</div>
                ...
                if (s.id === "quote") return (
                    <div key="quote" className="border-t border-current/10 px-5 py-12 sm:px-10 sm:py-16">
                        ...
                        <div className="mt-4 font-display text-3xl leading-[0.95] sm:text-4xl lg:text-5xl">
                            "{landingQuote.body}"
                        </div>
                ...
                if (s.id === "pricing") return (
                    <div key="pricing" className="border-t border-current/10 px-5 py-12 sm:px-10 sm:py-16">
                        ...
                        <h2 className="mt-3 font-display text-3xl leading-[0.95] sm:text-4xl">
                            PAY PER FLEET. <span style={{ background: accent, color: accentText, padding: "0 0.08em" }}>NO LOCK-IN.</span>
                        </h2>
                        <div className="mt-6 grid gap-3 sm:grid-cols-3">
                            {[
                                ...
                                <div key={tier.p} className={`border-2 p-5 ${i === 1 ? "" : ""}`} style={{
                ...
                if (s.id === "cta") return (
                    <div key="cta" className="px-5 py-14 sm:px-10 sm:py-20" style={{ background: accent, color: accentText }}>
                        ...
                        <p className="mt-5 max-w-xl font-mono text-sm leading-relaxed opacity-80">
                            No deck. No slides. We map your three highest-volume routes against a Cargobyte fleet,
                            then send you the cost diff in writing.
                        </p>
                        <button className="mt-8 border-2 px-7 py-3.5 font-mono text-xs tracking-wider" style={{ borderColor: accentText, color: accentText }}>
...
export default function Landing() {
    ...
    const [editingHero, setEditingHero] = useState(null); // { title, sub }
    const [sections, setSections] = useState(landingSections.map((s) => ({ ...s, enabled: true })));

    const theme = landingThemes.find((t) => t.id === themeId);
    const baseHero = landingHeroVariants.find((h) => h.id === heroId);
                ...
                <div className="relative mx-auto max-w-[1480px] px-6 lg:px-10">
                    ...
                    <div className="mt-3 grid gap-8 lg:grid-cols-12">
                        ...
                        <p className="max-w-md self-end font-mono text-sm leading-relaxed text-neutral-600 lg:col-span-4">
                            Reorder sections. Edit copy inline. Pick an accent. Then push to Figma, Framer
                            or Webflow with one click.
                        </p>
                    </div>
                </div>
            </section>
                            ...
                            <div className="border-2 border-black bg-white">
                                <div className="flex items-center justify-between border-b-2 border-black bg-black px-4 py-2">
                                    ...
                                    <button
                                        ...
                                        onClick={() => setEditingHero(editingHero ? null : { title: baseHero.title, sub: baseHero.sub, kicker: baseHero.kicker })}
                                    ...
                                    {sections.map((s, idx) => (
                                        <li key={s.id} className="border-b border-black/10 last:border-b-0">
                                            <div className="flex items-center gap-2 px-3 py-2.5">
                                                <span className="font-mono text-[10px] tracking-wider text-neutral-400 w-6">
                                                    {String(idx + 1).padStart(2, "0")}
                                                </span>
                                                <span className={`flex-1 font-mono text-[11px] tracking-wider ${s.enabled ? "text-black" : "text-neutral-400 line-through"}`}>
                                                    {s.label}
                                                </span>
                                                {s.required && <span className="border border-black px-1 py-0.5 font-mono text-[8px]">REQ</span>}
                                                <button
                                                    type="button"
                                                ...
                                                <button
                                                    ...
                                                    className="border border-black/20 p-1 transition hover:border-black disabled:opacity-30"
                                                >
                                                    {s.enabled ? <Eye className="h-3 w-3" /> : <EyeOff className="h-3 w-3 text-neutral-400" />}
                                                </button>
                                            </div>
                                        </li>
                                    ))}
                    ...
                    <div className="mb-10 flex flex-wrap items-end justify-between gap-4">
                        <div>
                            ...
                            <p className="mt-4 max-w-xl font-mono text-sm leading-relaxed text-white/55">
                                Push the live preview to Figma, Framer or Webflow with all sections, accent
                                token and copy intact. Tier-gated.
                            </p>
                        </div>
                        <button
